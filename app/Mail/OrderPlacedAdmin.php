...
class OrderPlacedAdmin extends Mailable implements ShouldQueue
{
    ...
    {
        $this->order = $order->loadMissing(['items', 'billingAddress', 'shippingAddress', 'workingGroup']);
        $this->renderer = app(TemplateRenderer::class);
        
        // Load template
        $this->template = MessageTemplate::where('slug', 'new-order-admin-email')
    ...
    public function envelope(): Envelope
    {
        ...
        $subject = $this->renderedContent['subject'] ?? 'New order #' . $this->order->id . ' received';
