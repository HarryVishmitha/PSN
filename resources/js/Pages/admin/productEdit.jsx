...
import 'highlight.js/styles/atom-one-dark-reasonable.css'; // Your chosen highlight.js theme

// Ensure hljs is available on the window object
...
const Productedit = ({ userDetails, workingGroups, categories, providers, productDetails }) => {
  ...
  useEffect(() => {
    if (productDetails) {
      // Set working group
      setSelectedWorkingGroup(productDetails.working_group_id || '');
      // Basic info
      setName(productDetails.name || '');
      setProductDescription(productDetails.description || '');
      setSeoTitle(productDetails.meta_title || '');
      setSeoDescription(productDetails.meta_description || '');
      // Map categories to the format required by ReactSelect
      const prodCategories = (productDetails.categories || []).map(cat => ({
        value: cat.id,
        label: cat.name,
      }));
      ...
      if (productDetails.pricing_method === 'standard') {
        setBasePrice(productDetails.price || '');
        // Setup variants if they exist
        if (productDetails.variants && productDetails.variants.length > 0) {
          setHasVariants(true);
          // Map each product variant to our expected shape
          const mappedVariants = productDetails.variants.map(variant => {
            // Find matching inventory for this variant
            const inv = (productDetails.inventories || []).find(i => i.product_variant_id === variant.id) || {};
            const hasSubvariants = variant.subvariants && variant.subvariants.length > 0;
            // Map subvariants if any
            const mappedSubvariants = (variant.subvariants || []).map(sub => ({
              ...
              inventory: {
                ...
                provider: inv.provider_id || '',
                unitDetails: inv.unit_details || '',
              },
            }));
            return {
              ...
              subvariants: mappedSubvariants,
            };
          });
          setVariants(mappedVariants);
        } else {
          // If there are no variants, assume global inventory is provided.
          setHasVariants(false);
          if (productDetails.inventories && productDetails.inventories.length > 0) {
            const inv = productDetails.inventories[0];
            setGlobalQuantity(inv.quantity || '');
            setGlobalReorderThreshold(inv.reorder_threshold || '');
            setGlobalProvider(inv.provider_id || '');
            setGlobalUnitDetails(inv.unit_details || '');
          }
        }
      } else if (productDetails.pricing_method === 'roll') {
        setPricePerSqft(productDetails.price_per_sqft || '');
      }

      // Map images to the state used by the uploader.
      if (productDetails.images && productDetails.images.length > 0) {
        const mappedImages = productDetails.images.map(img => ({
          src: img.image_url,
          isPrimary: img.is_primary === 1,
          // Since these images are already on the server, there's no File object.
          file: null,
          id: img.id,
        }));
  ...
  // Variants functions remain unchanged
  const addVariant = () => {
    setVariants([
      ...variants,
      {
  ...
  // Quill text editor and highlight.js setup
  ...
  // Optionally, save inputs to localStorage (if desired)
  useEffect(() => {
    const data = {
  ...
  const handleFormsubmit = (e) => {
    ...
    router.post(route('admin.editproduct', productDetails.id), data, {
      ...
      onError: (errors) => {
        setErrors(errors);
        setAlert({ type: 'danger', message: 'Failed to update product. Please check the errors.' });
        console.log(errors);
      },
      onFinish: () => {
        setLoading(false);
  ...
  return (
    <>
      <Head title="Edit Product - Admin Dashboard" />
      <Meta title="Edit Product - Admin Dashboard" description="Update product details" />
              ...
              <form onSubmit={handleFormsubmit} className="form-wizard-form">
                ...
                <fieldset className={`wizard-fieldset ${currentStep === 2 && "show"}`}>
                  ...
                  <div className="row gy-3">
                    ...
                    <div className="col-sm-6 mt-1">
                      ...
                      <textarea className={`form-control ${errors.seoTitle ? 'is-invalid' : ''}`} value={seoTitle} onChange={(e) => setSeoTitle(e.target.value)}></textarea>
                      {errors.seoTitle && (
                    ...
                    <div className="col-sm-6 mt-1">
                      ...
                      <textarea className={`form-control ${errors.seoDescription ? 'is-invalid' : ''}`} value={seoDescription} onChange={(e) => setSeoDescription(e.target.value)}></textarea>
                ...
                <fieldset className={`wizard-fieldset ${currentStep === 3 && "show"}`}>
                  ...
                  <div className="form-group d-flex align-items-center justify-content-end gap-8 mt-3">
                    <button onClick={prevStep} type="button" className="btn btn-neutral-500 border-neutral-100 px-32">Back</button>
                  ...
                  <div className="form-group mb-3">
                    ...
                    <div>
                      <label className="me-3">
                        <input type="radio" checked={pricingMethod === 'standard'} onChange={() => setPricingMethod('standard')} /> Piece Based (Standard)
                      </label>
                      <label>
                        <input type="radio" checked={pricingMethod === 'roll'} onChange={() => setPricingMethod('roll')} /> Roll Based
                      </label>
                    </div>
                  </div>
                  {pricingMethod === 'standard' && (
                    <>
                      <div className="form-group mb-3">
                        ...
                        <input type="number" className="form-control" placeholder="Enter base price" value={basePrice} onChange={(e) => setBasePrice(e.target.value)} />
                      ...
                      <div className="form-group mb-3">
                        ...
                        <div>
                          <label className="me-3">
                            <input type="radio" checked={!hasVariants} onChange={() => setHasVariants(false)} /> No
                          </label>
                          <label>
                            <input type="radio" checked={hasVariants} onChange={() => setHasVariants(true)} /> Yes
                          </label>
                        </div>
                      </div>
                      {!hasVariants && (
                        <div className="form-group">
                          ...
                          <div className="row">
                            <div className="col-md-3 mb-3">
                              ...
                              <input type="number" className="form-control" placeholder="Enter quantity" value={globalQuantity} onChange={(e) => setGlobalQuantity(e.target.value)} />
                            ...
                            <div className="col-md-3 mb-3">
                              ...
                              <input type="number" className="form-control" placeholder="Enter reorder threshold" value={globalReorderThreshold} onChange={(e) => setGlobalReorderThreshold(e.target.value)} />
                              {errors.globalReorderThreshold && <div className="invalid-feedback">{errors.globalReorderThreshold}</div>}
                            </div>
                            <div className="col-md-3 mb-3">
                              <label className="form-label">Provider*</label>
                              <select className="form-control" value={globalProvider} onChange={(e) => setGlobalProvider(e.target.value)}>
                            ...
                            <div className="col-md-3 mb-3">
                              ...
                              <textarea className="form-control" placeholder="Enter unit details" value={globalUnitDetails} onChange={(e) => setGlobalUnitDetails(e.target.value)} />
                            </div>
                          </div>
                        </div>
                      )}
                            ...
                            <div key={vIndex} className="tw-border p-3 mb-3 tw-border-gray-500 tw-rounded-lg">
                              <div className="row">
                                <div className="col-md-6 mb-3">
                                  ...
                                  <input type="text" className="form-control" placeholder="Variant Name (e.g., Size)" value={variant.name} onChange={(e) => handleVariantChange(vIndex, 'name', e.target.value)} />
                                ...
                                <div className="col-md-6 mb-3">
                                  ...
                                  <input type="text" className="form-control" placeholder="Variant Value (e.g., 18x24)" value={variant.value} onChange={(e) => handleVariantChange(vIndex, 'value', e.target.value)} />
                                ...
                                <div className="col-md-6 mb-3">
                                  ...
                                  <div className="tw-text-xs tw-text-yellow-500 mb-3">Define the adjustable price relative to the base price.</div>
                                  <input type="number" className="form-control" placeholder="Price Adjustment" value={variant.priceAdjustment} onChange={(e) => handleVariantChange(vIndex, 'priceAdjustment', e.target.value)} />
                              ...
                              {!variant.hasSubvariants && (
                                <>
                                  ...
                                  <div className="row">
                                    <div className="col-md-3 mb-3">
                                      <label className="form-label">Quantity*</label>
                                      <input type="number" className="form-control" placeholder="Enter quantity" value={variant.inventory.quantity} onChange={(e) =>
                                        handleVariantChange(vIndex, 'inventory', { ...variant.inventory, quantity: e.target.value })
                                      } />
                                      {errors[`variant_${vIndex}_quantity`] && <div className="invalid-feedback">{errors[`variant_${vIndex}_quantity`]}</div>}
                                    </div>
                                    <div className="col-md-3 mb-3">
                                      <label className="form-label">Reorder Threshold*</label>
                                      <input type="number" className="form-control" placeholder="Enter reorder threshold" value={variant.inventory.reorderThreshold} onChange={(e) =>
                                        handleVariantChange(vIndex, 'inventory', { ...variant.inventory, reorderThreshold: e.target.value })
                                      } />
                                      {errors[`variant_${vIndex}_reorderThreshold`] && <div className="invalid-feedback">{errors[`variant_${vIndex}_reorderThreshold`]}</div>}
                                    </div>
                                    <div className="col-md-3 mb-3">
                                      <label className="form-label">Provider*</label>
                                      <select className="form-control" value={variant.inventory.provider} onChange={(e) =>
                                        handleVariantChange(vIndex, 'inventory', { ...variant.inventory, provider: e.target.value })
                                      }>
                                        <option value="">Select Provider</option>
                                        {providers.map((prov) => (
                                          <option key={prov.id} value={prov.id}>{prov.name}</option>
                                    ...
                                    <div className="col-md-3 mb-3">
                                      <label className="form-label">Unit Details</label>
                                      <textarea className="form-control" placeholder="Enter unit details" value={variant.inventory.unitDetails} onChange={(e) =>
                                        handleVariantChange(vIndex, 'inventory', { ...variant.inventory, unitDetails: e.target.value })
                                      } />
                                    </div>
                                  </div>
                                </>
                              ...
                              <div className="mt-2">
                                <label>
                                  <input type="checkbox" checked={variant.hasSubvariants} onChange={(e) =>
                                  ...
                                  {variant.subvariants.map((subvariant, sIndex) => (
                                    <div key={sIndex} className="row mb-2 align-items-center mt-3">
                                      ...
                                      <div className="d-flex align-items-center gap-3 mb-3 mt-3">
                                        ...
                                        <button type="button" className="btn btn-danger btn-sm" onClick={() => removeSubvariant(vIndex, sIndex)}>
                                          <Icon icon="fluent:delete-24-regular" alt="delete" />
                                        </button>
                                      </div>
                                      <div className="col-md-3 mb-3">
                                        <label className="form-label">Subvariant Name*</label>
                                        <input type="text" className="form-control" placeholder="Enter subvariant name" value={subvariant.name} onChange={(e) =>
                                      ...
                                      <div className="col-md-3 mb-3">
                                        ...
                                        <input type="text" className="form-control" placeholder="Enter subvariant value (e.g., red, 18x24)" value={subvariant.value} onChange={(e) =>
                                      ...
                                      <div className="col-md-3 mb-3">
                                        ...
                                        <input type="number" className="form-control" placeholder="Enter price adjustment" value={subvariant.priceAdjustment} onChange={(e) =>
                                      ...
                                      <div className="col-md-3 mb-3">
                                        <label className="form-label">Quantity*</label>
                                        <input type="number" className="form-control" placeholder="Enter quantity" value={subvariant.inventory.quantity} onChange={(e) =>
                                          handleSubvariantChange(vIndex, sIndex, 'inventory', { ...subvariant.inventory, quantity: e.target.value })
                                        } />
                                      ...
                                      <div className="col-md-3 mb-3">
                                        <label className="form-label">Reorder Threshold*</label>
                                        <input type="number" className="form-control" placeholder="Enter reorder threshold" value={subvariant.inventory.reorderThreshold} onChange={(e) =>
                                          handleSubvariantChange(vIndex, sIndex, 'inventory', { ...subvariant.inventory, reorderThreshold: e.target.value })
                                        } />
                                      ...
                                      <div className="col-md-3 mb-3">
                                        <label className="form-label">Provider*</label>
                                        <select className="form-control" value={subvariant.inventory.provider} onChange={(e) =>
                                          handleSubvariantChange(vIndex, sIndex, 'inventory', { ...subvariant.inventory, provider: e.target.value })
                                        }>
                                          <option value="">Select Provider</option>
                                          {providers.map((prov) => (
                                            <option key={prov.id} value={prov.id}>{prov.name}</option>
                                      ...
                                      <div className="col-md-3 mb-3">
                                        <label className="form-label">Unit Details</label>
                                        <textarea className="form-control" placeholder="Enter unit details" value={subvariant.inventory.unitDetails} onChange={(e) =>
                                          handleSubvariantChange(vIndex, sIndex, 'inventory', { ...subvariant.inventory, unitDetails: e.target.value })
                                        } />
                                      </div>
                                    </div>
                                  ))}
                                  <button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => addSubvariant(vIndex)}>
                                    Add Subvariant
                                  </button>
                                </div>
                              )}
                              <button type="button" className="btn btn-danger btn-sm mt-2" onClick={() => removeVariant(vIndex)}>
                                Remove Variant
                              </button>
                            </div>
                          ))}
                  ...
                  {pricingMethod === 'roll' && (
                    <>
                      <div className="form-group mb-3">
                        ...
                        <input type="number" className="form-control" placeholder="Enter price per square foot" value={pricePerSqft} onChange={(e) => setPricePerSqft(e.target.value)} />
                    ...
                    <div className="upload-image-wrapper d-flex align-items-center gap-3 flex-wrap">
                      <div className="uploaded-imgs-container d-flex gap-3 flex-wrap">
                        {uploadedImages.map((image, index) => (
                          <div key={index} className="position-relative border radius-8 overflow-hidden border-dashed bg-neutral-50 tw-h-[250px] tw-w-[250px]">
                            <button type="button" className="position-absolute top-0 end-0 z-1 text-2xxl me-8 mt-8 d-flex" onClick={() => removeImage(image.src)}>
                            ...
                            <div className="position-absolute bottom-0 start-0 z-1 p-1 tw-bg-slate-300">
                              <input type="radio" name="primaryImage" checked={!!image.isPrimary} onChange={() => setPrimaryImage(index)} /> <span className="text-black fw-semibold">Set as Primary</span>
                            </div>
                            <div className="position-absolute bottom-0 end-0 z-1 p-1 d-flex flex-column">
                              <button type="button" onClick={() => moveImageUp(index)} className="btn btn-sm btn-secondary mb-1">
                                <Icon icon="akar-icons:arrow-up" />
                              </button>
                              <button type="button" onClick={() => moveImageDown(index)} className="btn btn-sm btn-secondary">
                                <Icon icon="akar-icons:arrow-down" />
                              </button>
                            </div>
                            <img className="w-100 h-100 object-fit-cover" src={image.src} alt="Uploaded Preview" />
                          </div>
                        ))}
                      </div>
                      <label className="upload-file-multiple tw-h-[250px] tw-w-[250px] border radius-8 overflow-hidden border-dashed bg-neutral-50 d-flex align-items-center flex-column justify-content-center gap-1" htmlFor="upload-file-multiple">
                        ...
                        <input id="upload-file-multiple" type="file" hidden multiple onChange={handleFileChange} className="tw-h-[250px] tw-w-[250px]" />
                      </label>
                    </div>
                  </div>
                  <div className="form-group d-flex align-items-center justify-content-end gap-8 mt-3">
                      ...
                      <div className="d-flex flex-wrap gap-3 ">
                        {imagesInfo.length > 0 ? (
                          imagesInfo.map((image, index) => (
                            <img key={index} src={image.src} alt={`Image ${index + 1}`} className="tw-h-[250px] tw-w-[250px] object-cover border rounded" />
                          ))
                        ) : (
              ...
              <div className="modal-body px-24">
                <form onSubmit={handleAddCategory}>
                  ...
                  <div className="mb-3">
                    ...
                    <textarea id="categoryDescription" className="form-control" placeholder="Enter category description (optional)" value={newCategoryDescription} onChange={(e) => setNewCategoryDescription(e.target.value)} />
                  </div>
